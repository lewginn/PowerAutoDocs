...
- Step 2 injects the PAT: `sed -i "s|REDACTED|${WIKI_PAT}|g" $(DOC_GEN_CONFIG_DIR)/doc-gen.config.yml`. The `|` delimiter is load-bearing — ADO PATs are base64 (`A-Za-z0-9+/=`), and a PAT containing `/` closes an `s///` expression early, so sed aborts and leaves the literal `REDACTED` in the config (which then trips the PAT guard at `index.ts:330`). `${WIKI_PAT}` is the shell variable from the step's `env:` block, not the ADO `$(WIKI_PAT)` macro, so the secret is never pasted into the generated script file on the agent.
...
- Step 4 commits the cache folder (`PowerAutoDocs/.powerautodocs-cache/`, `cache.dir`) back to the branch, so ADO's ephemeral agent doesn't start cold every run — see [Caches](#caches) below. Requires the ADO Build Service account to have Contribute permission on the repo; without it the push is rejected with `TF401027: You need the Git 'GenericContribute' permission` (`GenericContribute` is Contribute's on-the-wire name). Documentation still generates and publishes either way — step 4 only persists the cache. **Note this step used to fail the whole run** (`git push` → exit 128 → red build) despite the docs having published fine; `continueOnError` is what makes the "run still succeeds" claim actually true. Three details in that step are load-bearing:
  - It pushes to **`$(Build.SourceBranch)`** (the full `refs/heads/x` ref), *not* `$(Build.SourceBranchName)`, which is only the last path segment — on `feature/docs` that resolves to `docs` and pushes the cache to a new, wrong branch with no error. Both forms agree on `main`, so the bug is invisible there.
  - It rebases onto the fetched ref first, because a commit landing on the branch during a long multi-solution run would otherwise make the push a rejected non-fast-forward.
  - It is `continueOnError: true`. The cache is a re-run optimisation, not an output; a failed push shouldn't red a run whose documentation published fine. The step still logs warnings naming the likely cause, so the failure stays visible.
