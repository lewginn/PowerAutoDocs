...
| Rule | Why |
|------|-----|
| **Never bump `package.json` `version`, and never run `npm publish` locally.** | Releases are Lewis's call and are automated — `.github/workflows/npm-publish.yml` runs `npm ci && npm run build && npm publish` on a GitHub Release (`types: [created, published]`). A manual publish or a stray bump desynchronises the tag, the Release, and the registry. Current version is `1.4.0`. |
...
- **No CI runs on pull requests.** Nothing gates a merge. *Why:* `npm run build` is only ever executed by the publish workflow — i.e. *after* a release is cut. Running it locally before you push is the only compile gate that exists. See [process.md](process.md).
