...
| **Never add an npm dependency without asking.** | Every dep ships to clients via `npx powerautodocs@latest` and is a supply-chain and install-time cost on ephemeral ADO agents. Note `commander`, `handlebars`, `zod`, `adm-zip` and `glob` are declared in `package.json` but imported nowhere in `src/` — their presence is **not** evidence they are sanctioned. Do not start using them. |
...
- **CI runs on pull requests, but it is a floor, not a proof.** `.github/workflows/ci.yml` typechecks, builds and tests every PR and every push to `main`; `npm-publish.yml` repeats the checks before publishing. *Why it doesn't let you skip verification:* the suite covers every parser and every renderer up to the `DocNode` boundary — and stops there. It does **not** exercise `DocxSerializer`, `PdfSerializer`, `publisher/*`, Mermaid rendering, the AI providers or the wiki publisher, so **no test in this repo has ever produced a `.docx`, a `.pdf` or a wiki page.** A green PR says the tested fraction didn't regress; it says nothing about the artifact. The backtick bug is the proof: every renderer test passed while the `.docx` shipped literal backticks. See [process.md](process.md).
