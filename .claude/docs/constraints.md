...
- **CI runs on pull requests, but it is a floor, not a proof.** `.github/workflows/ci.yml` typechecks, builds and tests every PR and every push to `main`; `npm-publish.yml` repeats the checks before publishing. *Why it doesn't let you skip verification:* the suite covers DocNode serialisation, `rendererUtils`, the ERD generator and the fixtured parsers — **not** Word/PDF output, Mermaid rendering, the AI providers or the wiki publisher. A green PR says the tested fraction didn't regress; it says nothing about the artifact. See [process.md](process.md).
