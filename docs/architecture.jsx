...
const layers = [
  ...
  },
  {
    ...
    components: [
      ...
      { name: "GitHub Actions npm Publish", icon: "🚀", detail: "npm-publish.yml workflow. Triggers on GitHub Release created or published, plus workflow_dispatch as a manual escape hatch. Runs npm ci → typecheck → test → build → npm publish. NODE_AUTH_TOKEN from NPM_TOKEN repo secret. Replaces manual npm publish from local machine.", tags: ["CI/CD"], done: true, moscow: "S" },
      { name: "CI Pipeline", icon: "✅", detail: "ci.yml — runs on every pull request and every push to main. Node 20, npm ci → npm run typecheck → npm run build → npm test. Typecheck covers src/ and tests/ via tsconfig.test.json; the build only sees src/. Concurrency cancels superseded runs on branches but never on main. Before this, npm run build ran only after a release was cut.", tags: ["CI/CD"], done: true, moscow: "M" },
      { name: "Vitest Test Suite", icon: "🧪", detail: "Vitest, chosen for zero-config pure-ESM/NodeNext support. Tests live in tests/ outside the build's src/ scope, so they never ship to npm. Covers MarkdownSerializer (DocNode → markdown), rendererUtils, erdGenerator and the fixtured parsers — no mocks anywhere. Fixtures are hand-written for a fictional Contoso solution: never derived from real client data. Word/PDF binaries, Mermaid rendering, AI providers and the wiki publisher are deliberately out of scope until those seams support injection.", tags: ["Testing"], done: true, moscow: "M" },
