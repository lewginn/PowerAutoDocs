...
const layers = [
  ...
  },
  {
    ...
    components: [
      ...
      { name: "Company Word Template", icon: "🏢", detail: "Renders the document into your own branded Word template rather than styling one from scratch, so the output arrives with the company logo, headers, footers, fonts and page setup already on it. Needs no preparation: point it at the company .docx and run. Mark a spot with a {{content}} placeholder if you want to keep a cover page or back page around the documentation. Works against any Word template, because the heading styles it hooks into are Word built-ins rather than anything specific to one brand. Name the template's own table and bullet styles and the generated tables and lists adopt them too, so the document reads as one of the company's rather than as something a tool produced.", tags: ["Optional"], done: true, moscow: "S" },
...
const decisions = [
  ...
  { q: "Company Word template?", a: "patchDocument() into a client-supplied .docx or .dotx, no new dependency", reason: "The blocker on using the tool internally: the themed output could only ever be a lookalike of a corporate template - no logo, no branded headers or footers, no cover page. The document is now injected into the real template instead, at a {{content}} placeholder, using patchDocument() from the docx library we already ship. That it needs no new dependency is what made it cheap: every dependency is paid on every ephemeral ADO agent run, forever. The inversion is the point - buildDocument() decides fonts, page size, margins and footers, whereas here the template already decided all of them and we contribute only the body. It generalises to any template rather than one we prepared, because Heading1-Heading9, ListParagraph and TableGrid are OOXML built-ins present in every Word file regardless of its design or UI language; only branded style names are template-specific, and those are config (output.wordTemplateStyles.table) with the existing wordTheme as fallback. Content is injected at a placeholder rather than by replacing the document body, so a template's own cover and back pages survive and the section properties carrying the header/footer references never have to be reconstructed. Bullets are the one place this diverges: a Word list is a reference into numbering.xml, which under a template is the template's file, so a native bullet resolves to whatever list that template defines at that id - verified against a real company template whose numId 1 is a decimal list, which turned every bulleted flow action into 1, 1.1, 1.1.1. Under a template the glyph is therefore written directly with an explicit indent, giving up list semantics to be correct against a template nobody has seen. Two things then needed to edit a part inside the .docx that the docx library will not touch: the setting that makes Word refresh the table of contents on open (it lives in settings.xml, which comes from the template), and substituting a placeholder into an unprepared template so no manual preparation step is needed - a step that failed twice in the first ten minutes of real use, because a prepared and an unprepared template are indistinguishable by name. Both are single-part edits, and both routes to a library were closed: jszip is only a transitive dependency of docx, and adm-zip is a devDependency that never reaches a client agent. So the ZIP read/write is hand-rolled on node:zlib in docxZip.ts, a few hundred lines of well-specified format against a permanent per-run install cost for every client. Entries nobody edited are copied through as their original compressed bytes rather than re-deflated, so a 2.4MB document with 28 embedded images keeps every image byte-identical and only the edited XML part is regenerated. A .dotx is accepted alongside a .docx, and is in practice the likelier of the two - it is what Word produces when you save a template, so it is what a company template usually is. The packages are otherwise identical, differing only in the content type declared for the main part, which makes rewriting that one declaration on the way in the entire conversion. Without it the tool hands back a file named .docx whose package declares itself a template: patching succeeds, document.xml is correct, and the mismatch is visible nowhere except [Content_Types].xml." },
  { q: "A template and a theme together?", a: "They compose - the template wins what it defines, the theme fills what Word cannot express", reason: "Not either/or, and the seam between them leaked until a real run exposed it. A template owns fonts, heading styles, page size, margins, headers and footers; wordTheme is bypassed for all of it. But Word has no notion of a code-chip style, and table column widths are measured against a font size rather than inherited, so the theme still has to supply those. The failure mode was the gap between: with no wordTheme block configured at all, accentColor defaults to our blue and codeColor derives from it, so a purple-branded document came back with 1,226 runs of navy code text nobody had chosen. The rule is now derived-versus-explicit rather than on-versus-off - under a template, anything we would otherwise derive from the accent (code colour, and table fills when no table style is named) derives from neutral grey instead, while any colour the client actually configured still wins, because they asked for it and the template cannot express it either way. The real defect underneath was documentation: the precedence lived in code and was written down nowhere, so the only way to discover it was to generate a document and inspect the colours." },
