...
const layers = [
  ...
  },
  {
    ...
    components: [
      ...
      { name: "Vitest Test Suite", icon: "🧪", detail: "Vitest, chosen for zero-config pure-ESM/NodeNext support. Tests live in tests/ outside the build's src/ scope, so they never ship to npm. 1090 tests in ~3s, covering every module with runtime behaviour: all 17 parsers, all 14 renderers, MarkdownSerializer and DocxSerializer (both asserted against real generated output), wordTheme, erdGenerator, config/loader, the wiki and Word assemblers, the wiki publisher, the pipeline entry point and the AI enrichment layer. Fixtures are hand-written for fictional Contoso/Acme solutions: never derived from real client data. Still no mocks anywhere — the wiki publisher, AI provider and entry point were refactored to accept injected seams rather than be mocked around, on the principle that a test needing a mock is asking for a refactor first. Real browser Mermaid rendering and the PDF path remain out of scope by decision, not omission.", tags: ["Testing"], done: true, moscow: "M" },
